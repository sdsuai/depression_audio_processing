...
from exp_run.models_pytorch import CustomAttentionTransformer as CustomAttentionTransformer
...
def forward(model, generate_dev, data_type):
    ...
    for data in generate_dev:
        ...
        with torch.no_grad():
            batch_output, batch_att = get_output_from_model(model=model,
                                                 data=batch_data)
            if accumulated_att is None:
                accumulated_att = torch.mean(batch_att['layer_0'], dim=0)
            else:
                accumulated_att += torch.mean(batch_att['layer_0'], dim=0)
        counter += 1
        # Append data
    ...
    return results_dict, accumulated_att
...
def evaluate(model, generator, data_type, class_weights, comp_res, class_num,
             ...
             hidden_test=False, placeholder=None):
    """
    ...
    # Forward
    results_dict, attention = forward(model=model,
    ...
    # Find all existing files with the base name
    existing_files = [f for f in os.listdir('.') if f.startswith(base_name) and f.endswith(extension)]

    # Extract the counters from the existing file names
    counters = [int(re.search(r'_(\d+)', f).group(1)) for f in existing_files if re.search(r'_(\d+)', f)]

    # Determine the next counter value
    next_counter = max(counters) + 1 if counters else 1

    # Construct the new file name
    file_name = f"{base_name}_{next_counter}{extension}"

    # Save the new pickle file
    with open(file_name, 'wb') as f:
        pickle.dump(attention, f)

    with open("epoch_counter.txt", 'a') as file:
        if placeholder is not None:  # Check if placeholder is provided
            file.write(f"Epoch: {placeholder}, Counter: {next_counter}\n")
...
def update_complete_results(complete_results, avg_counter, placeholder,
    ...
    best_scores = record_top_results(complete_results,
                                     best_scores,
                                     placeholder)
    all_f_score.append((complete_results[21],complete_results[22],complete_results[24]))
...
def final_organisation(scores, train_pred, val_pred, df, patience, epoch,
    ...
    destination_dir = current_dir + '/daic'

    # Check if the destination directory already exists, and if so, remove it
    if os.path.exists(destination_dir):
        shutil.rmtree(destination_dir)

    # Copy the directory

    '''shutil.copytree(workspace_files_dir, destination_dir)
...
def get_output_from_model(model, data):
    ...
    current_data = mu.create_tensor_data(data,
                                         cuda)
   # model_output = model(current_data)
    #print(f"Model output: {model_output}")

    output = model(current_data)
    att = model.get_attention_details()

    if cuda:
        output, att["layer_0"] = output.to("cpu"), att["layer_0"].to("cpu")
        # output = output.cpu()
        # att = att.cpu()
        ...
        if begin_evaluate:
            ...
            if placeholder % patience == 0:
                ...
                final_organisation(best_scores,
                                   ...
                                   data_saver)
                break
    print(complete_results, best_scores)
